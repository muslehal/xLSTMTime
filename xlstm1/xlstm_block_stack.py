...
# from .blocks.mlstm.block import mLSTMBlock, mLSTMBlockConfig
# from .blocks.slstm.block import sLSTMBlock, sLSTMBlockConfig
# from .components.ln import LayerNorm  # You can remove this import if you are not using LayerNorm
...
#         self.blocks = self._create_blocks(config=config)
#         if config.add_post_blocks_norm:
#             self.post_blocks_norm = nn.BatchNorm1d(num_features=config.embedding_dim)
#         else:
#             self.post_blocks_norm = nn.Identity()
...
#         x = self.post_blocks_norm(x.transpose(1, 2)).transpose(1, 2)  # Adjust for BatchNorm1d
